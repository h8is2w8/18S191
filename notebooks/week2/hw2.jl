...
@bind l_box Slider(1:10; default=1)
...
# ╔═╡ 71f500fe-7e86-11eb-193e-b3fdf2f89f86
function edge(n, k)
	if n < 1
		1
	elseif n > k
...
function extend(M::AbstractMatrix, i, j)
	M[edge(i, size(M, 1)), edge(j, size(M, 2))]
end
...
function box_blur(v::AbstractArray, l)
	...
	return [mean([extend(v, i+k) for k=-l:l]) for i=1:size(v, 1)]
end
...
[philip_head convolve(philip_head, K_test)]
...
# ╔═╡ 4fdac958-7e93-11eb-39a2-51cdc7c3d402
function gauss_2d_kernel(σ, l)
...
function with_sobel_edge_detect(image)
	...
	im1 = convolve(image, X)
	im2 = convolve(image, Y)
